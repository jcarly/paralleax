...
export class StoriesService {
  ...
  list(): Story[] { return [...this.stories.values()].map((story) => structuredClone(story)); }
  get(id: string): Story {
  ...
  createInteraction(storyId: string, input: CreateInteractionDto): Story {
    ...
    if (input.parentId && !story.interactions.some((item) => item.id === input.parentId)) throw new NotFoundException('Parent interaction not found');
  ...
  updateInteraction(storyId: string, interactionId: string, input: UpdateInteractionDto): Story {
    ...
    Object.assign(interaction, input); return this.touch(story);
  }
