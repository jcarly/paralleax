...
export class StoriesService implements OnModuleInit {
  ...
  async createInteraction(storyId: string, input: CreateInteractionDto): Promise<Story> {
    return this.update(storyId, (story) => {
  ...
  private async update(id: string, mutation: (story: Story) => Story): Promise<Story> {
    const updated = await this.repository.mutate(id, (story) => {
  ...
  private async seed() {
    ...
    const current = await this.update(story.id, (item) => {
    ...
    await this.update(story.id, (final) => {
