...
function mockProgressiveStoryLoading() {
  ...
  vi.mocked(api.getStoryEditorBootstrap).mockImplementation(async () => {
    ...
    return storyProjectionBootstrap(loadedStory);
  });
  vi.mocked(api.getStoryEditorContextPage).mockImplementation(async (_id, page, pageSize) => {
    return storyProjectionContextPage(requiredLoadedStory(loadedStory), page, pageSize);
  });
  vi.mocked(api.getStoryEditorInteractionPage).mockImplementation(async (_id, page, pageSize) => {
    return storyProjectionInteractionPage(requiredLoadedStory(loadedStory), page, pageSize);
  });
  vi.mocked(api.getStoryEditorTriggerPage).mockImplementation(async (_id, page, pageSize) => {
    return storyProjectionTriggerPage(requiredLoadedStory(loadedStory), page, pageSize);
  });
  vi.mocked(api.getStoryEditorInteractionContentPage).mockImplementation(
    async (_id, page, pageSize) =>
      storyProjectionInteractionContentPage(requiredLoadedStory(loadedStory), page, pageSize),
  );
  vi.mocked(api.getStoryEditorTriggerContentPage).mockImplementation(async (_id, page, pageSize) =>
    storyProjectionTriggerContentPage(requiredLoadedStory(loadedStory), page, pageSize),
  );
}
